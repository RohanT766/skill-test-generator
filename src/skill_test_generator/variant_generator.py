...
def _extract_json(text: str) -> dict:
    """Extract a JSON object from LLM output, handling markdown fences and JS-isms."""
...
async def generate_variant(
    ...
    """Full pipeline: design → generate code → write to disk.

    Returns the variant spec augmented with generation metadata.
    """
    if slug is None:
    ...
    if spec_path.exists():
        logger.info("Variant '%s' already has spec, loading from disk", slug)
        spec = json.loads(spec_path.read_text())
    else:
        logger.info(
            "Designing variant for skill: %s (variant %d/%d)",
            skill.name,
            variant_index,
            total_variants,
        )
    ...
    code_path = variant_dir / "generated_code.json"
    if code_path.exists():
        logger.info("Variant '%s' already has generated code, loading", slug)
        code_files = json.loads(code_path.read_text())
    else:
        logger.info("Generating code for variant: %s", slug)
        code_files = await generate_variant_code(client, spec, model)

    errors = _validate_code_files(code_files)
    if errors:
        logger.warning("Validation issues for '%s', regenerating: %s", slug, errors)
        code_files = await generate_variant_code(
            client,
            spec,
            model,
            fix_context="Validation errors:\n" + "\n".join(f"- {e}" for e in errors),
        )

    code_path.write_text(json.dumps(code_files, indent=2))

    written = apply_variant_code(variant_dir, code_files, spec=spec)
    logger.info("Applied %d files to variant '%s'", len(written), slug)
...
async def generate_all_variants(
    ...
    """Legacy: design + code-gen all variants (kept for backward compatibility)."""
    client = anthropic.AsyncAnthropic(api_key=anthropic_api_key)
    semaphore = asyncio.Semaphore(concurrency)
    results: list[dict] = []

    async def gen_skill_variants(skill: SkillDefinition) -> list[dict]:
        skill_results: list[dict] = []
        prior_scenarios: list[str] = []
        base_slug = _slugify(skill.name)
            ...
            async with semaphore:
                ...
                for attempt in range(max_gen_retries + 1):
                    try:
                        ...
                        scenario_desc = result.get(
                            "scenario", result.get("description", "")
                        )
                        if scenario_desc:
                            prior_scenarios.append(
                                f"{result.get('title', slug)}: {scenario_desc}"
                            )
                        skill_results.append(result)
                        break
                    except Exception as e:
                        logger.warning(
                            "Variant generation attempt %d/%d for '%s' (v%d) failed: %s",
                            attempt + 1,
                            max_gen_retries + 1,
                            skill.name,
                            vi,
                        ...
                        stale_code = variants_dir / slug / "generated_code.json"
                        if stale_code.exists():
                            stale_code.unlink()
                        if attempt >= max_gen_retries:
                            logger.error("Giving up on variant '%s'", slug)

        return skill_results

    tasks = [gen_skill_variants(skill) for skill in skills]
    for skill_results in await asyncio.gather(*tasks):
        results.extend(skill_results)

    total_expected = len(skills) * specs_per_skill
    logger.info("Generated %d/%d variants", len(results), total_expected)
    return results
